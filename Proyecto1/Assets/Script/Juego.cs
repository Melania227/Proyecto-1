...
public class Juego : MonoBehaviour
{
    ...
    bool verificarFila(int filaAct_Aux, int columnaAct_Aux, int columnaMax_Aux, List<int> Fila, List<List<int>> pistaFilas) {
        int contadorDe1s = 0;
        if (pistaFilas[filaAct_Aux].Count == 0)
        {
            return false;
        }
    ...
    void rellenaMatriz(int[,] matrizLogica) {
    ...
    void Start()
    {
        lector.Leer();
        datos = lector.datos;
        int[,] matrizLogica = new int[datos.getX(), datos.getY()];
        rellenaMatriz(matrizLogica);
        impresionMatriz(matrizLogica);
        marcar(matrizLogica);
        impresionMatriz(matrizLogica);
    }
